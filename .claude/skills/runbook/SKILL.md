...
**Title and standfirst** — `# Runbook — <Workflow name>`, then one italic line stating what the
document is for, then a line addressing the operator directly.
...
**Where each item goes** *(optional)* — When a workflow routes into more than one destination,
a short table of route, condition and destination.

**How the score is worked out** *(optional)* — When routing depends on a calculation, show it.
The operator will be asked why something scored what it did.

**The checks it runs** — A numbered table of every check, and precisely what makes each one fire.
State that these run in code rather than in the model.
...
version changes, testing platform upgrades before they are taken. Mention that the test suite runs
after every change, if one exists.
...
**Footer** — One italic line distinguishing the demonstration version from a client build.
