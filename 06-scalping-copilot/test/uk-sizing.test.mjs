...
import {
  sizeBothModels, stakeAcrossPointSizes, marginFraction, minStopForMargin,
} from '../src/risk.js';
...
test('margin fraction is m*r/s, independent of account size and price level', () => {
  const s = 3.00 / 4391;
  near(marginFraction(0.05, 0.01, s), 0.7318, 0.0005);
  // Same stop as a FRACTION of price gives the same margin at any price level.
  near(marginFraction(0.05, 0.01, 3.00 / 4391), marginFraction(0.05, 0.01, 1.50 / 2195.5), 1e-9);
});

test('the same DOLLAR stop gets more expensive as gold rises', () => {
  const at2000 = marginFraction(0.05, 0.01, 3.00 / 2000);
  const at4391 = marginFraction(0.05, 0.01, 3.00 / 4391);
  assert.ok(at4391 > at2000 * 2,
    'a $3 stop used a third of the account at $2,000 gold and three quarters at $4,391');
